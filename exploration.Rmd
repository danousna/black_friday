...
X.cor <- round(cor(X), 2)
# Reorder
hc <- hclust(as.dist((1 - X.cor) / 2))
X.cor <- X.cor[hc$order, hc$order]
# Get upper tri
X.cor[lower.tri(X.cor)] <- NA 
# Melt
X.cor_melted <- melt(X.cor, na.rm = T)
# Plot
ggplot(X.cor_melted, aes(Var2, Var1, fill = value)) +
...
# Avec la méthode du coude, on prend k = 2 classes.
bf.kmeans <- kmeans(X, 2)

plot(X, col=bf.kmeans$cluster)
