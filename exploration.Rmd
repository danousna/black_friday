...
user_train_cat <- as.factor(as.matrix(purchase_sum[sapply(purchase_sum['User_ID'], function(x) {x %in% rownames(user_train)}),]['Gender']))
user_test_cat <- as.factor(as.matrix(purchase_sum[sapply(purchase_sum['User_ID'], function(x) {x %in% rownames(user_test)}),]['Gender']))
pr <- knn(user_train,user_test,cl=user_train_cat,k=9)
tab <- table(pr,user_test_cat)
tab
