...
```{r}
bf.numeric <- na.omit(bf[, -c(1, 2, 3, 4, 6, 7)])
bf.scaled <- scale(bf.numeric, center=T, scale=T)
bf.corr <- round(cor(bf.scaled), 2)
bf.corr_melted <- melt(bf.corr)
ggplot(data = bf.corr_melted, aes(x=Var1, y=Var2, fill=value)) + 
...
```{r}
bf.pca <- princomp(bf.scaled)
bf.kmeans <- kmeans(bf.pca$scores[, c(1,2)], 10)
plot(bf.pca$scores[, c(1,2)], col=bf.kmeans$cluster)
```